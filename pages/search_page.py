...
class SearchPage:
    ...
    def __init__(self, page: Page):
        ...
        # Grid (/search)
        self.grid_first_title = page.locator(
            "a.full-unstyled-link .card__heading__product-title"
        ).first
    ...
    def _submit_and_wait_grid(self, text: str) -> None:
        ...
        self.page.wait_for_selector(
            "a.full-unstyled-link .card__heading__product-title, p.alert.alert--warning",
            state="visible",
            timeout=10_000,
        )
    ...
    def search_product(self, text: str | None = None) -> str:
        ...
        expect(self.grid_first_title).to_be_visible(timeout=5000)
        return (self.grid_first_title.text_content() or "").strip()
    ...
    def run_test_search_existing_product(self) -> None:
        with allure.step(f"Run test for EXISTING product '{search_text}'"):
            first_title = self.search_product(search_text).lower()
            assert search_text.lower() in first_title, (
                f"Expected '{search_text}' in '{first_title}'"
            )

    def run_test_search_notexisting_product(self) -> None:
        with allure.step(f"Run test for NOT existing product '{search_text_invalid}'"):
            message = self.search_invalid_product(search_text_invalid)
            assert "no results found" in message.lower(), (
                f"Expected 'No results found' in '{message}'"
            )